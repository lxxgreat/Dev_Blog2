...
class UpYun(object):
    ...
    # 如缩略图间隔标志符为 ! ，密钥为 bac，上传文件路径为 /folder/test.jpg ，那么该图片的对外访问地址为：
    ...
    def writeFile(self, path, data, auto=False, headers={}, metadata={}):
        ...
        if resp.status == 200:
            ...
            self._tmp_info['x-upyun-file-type'] = resp.getheader(
                'x-upyun-file-type')
            return True
        else:
            return False
    ...
    # 获取文件信息
    # path 文件的远程路径
    # 返回格式为 {'date': unix time, 'type': file | folder, 'size': file size} 或
    # None
